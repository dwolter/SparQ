...
/** 
 * Qualifier backend for double cross calculus (dcc). 
...
std::string getRelation(const entity& o1, const entity& o2, const entity& o3) {
  string s = getDCC_Relation_String(o1.x, o1.y,
				    o2.x, o2.y,
				    o3.x, o3.y);
  
  if (s == ":error") {
    std::cerr << "DCC calculus not defined for the given configuration.\n";
    std::exit(1);
  }
...
const char* dcc_qualify(const char* bla, double a, double b, double c, double d, double e, double f)
{
