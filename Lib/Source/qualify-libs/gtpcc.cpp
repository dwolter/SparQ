...
GTPCC::GTPCC( int m ) {
    // Intialize GTPCC with Origin = [-1,0] and Relatum = [0,0]
    GTPCCConstruct( m, 0.0, 0.0, -1.0, 0.0);
}


GTPCC::GTPCC() {
    // Initialize GTPCC with default scale = 2
    GTPCCConstruct( 2, 0.0, 0.0, -1.0, 0.0 );
}

void GTPCC::GTPCCConstruct( int m,
                            float relatumX, float relatumY,
                            float originX=0, float originY=0 ) {
    BB_DBG(5) << "GTPCC (Constructor): Entering construction ..." << endl;
    Init();
    m_iM      = m;
    ...
    BB_DBG(6) << "GTPCC (Constructor): " << endl
    << "Scale m = " << m_iM << endl
    << "Using Relatum ("
    << relatumX << ", " << relatumY
    << ") and Origin: "
    << originX << ", " << originY << ")" << endl;
    BB_DBG(6) << "GTPCC (Constructor): Init Phase ..." << endl;
}
...
// SET/GET ORIGIN
// =================================================================================
int GTPCC::setOrigin( float x, float y) {
    m_fOriginX = x;
    m_fOriginY = y;
    m_bOriginSet = true;
...
int GTPCC::setOrigin( Point ptOrigin ) {
    m_ptOrigin = ptOrigin;
    BB_DBG(9) << "Origin set to : " << m_ptOrigin << endl;
    return true;
}
int GTPCC::getOrigin( float &x, float &y) {
    if( !m_bOriginSet )
        return false;
    x = m_fOriginX;
    y = m_fOriginY;
...
// SET/GET ORIGIN
// =================================================================================
int GTPCC::setRelatum( float x, float y) {
    m_fRelatumX = x;
    m_fRelatumY = y;
    m_bRelatumSet = true;
...
int GTPCC::setRelatum( Point ptRelatum ) {
    m_ptRelatum = ptRelatum;
    BB_DBG(9) << "Relatum set to : " << m_ptRelatum << endl;
    return true;
}

int GTPCC::getRelatum( float &x, float &y) {
    if( !m_bRelatumSet )
        return false;
    x = m_fRelatumX;
    y = m_fRelatumY;
...
void GTPCC::setLinear( bool bLinear ) {
    m_bIsLinear = bLinear;
}
...
// =================================================================================
int GTPCC::getMaxSectors() {
    if( m_iM > 0 ) {
        return (2*m_iM*4*m_iM+3);  // number of base relations with special cases
    }
    return 0;
}

int GTPCC::getMaxSectors( int m ) {
    if( m > 0 ) {
        return (2*m*4*m+3);
    }
    return (-1);
 ...
 */
std::vector< std::vector< int > >  GTPCC::getSectorTupel( float x, float y ) {
    Point ptReferent( x,y );
    return getSectorTupel( ptReferent );
}

std::vector< std::vector< int > >  GTPCC::getSectorTupel( Point referent ) {
    ...
    */
    ...
    BB_DBG(4) << endl;
    // Fill result vector m_GTPCC_Result
    for( unsigned int i=0; i<vDistanceSegments.size(); i++) {
...
std::vector< std::vector< int > > GTPCC::getNeighborhoodRelations( std::vector< int > relation ) {
    std::vector< std::vector< int > > result;
    std::vector< int > cnh_relation;

    int maxDist = 2*m_iM;                // GPCC [0..4m-1]
    int maxRot  = 4*m_iM;                // GPCC [1..8m]
    ...
    if( (relation[0]==0) && (relation[1]==0)) {
        ...
        result.push_back( cnh_relation );
        cnh_relation[0]=m_iM;
        cnh_relation[1]=maxRot/2;
        result.push_back( cnh_relation );
        cnh_relation[0]=m_iM;
        cnh_relation[1]=maxRot/2+1;
        result.push_back( cnh_relation );
        cnh_relation[0]=m_iM+1;
        cnh_relation[1]=maxRot/2;
        result.push_back( cnh_relation );
        cnh_relation[0]=m_iM+1;
        cnh_relation[1]=maxRot/2+1;
...
// =================================================================================
void GTPCC::Init () {
    BB_DBG(5) << ">>>>>>> GTPCC (Init) ..." << endl;

    m_GTPCC_Result.clear();
...
// =================================================================================
int GTPCC::calculateRadii( Point ptReferent ) {
 ...
 */
int GTPCC::calculateTheta( Point ptReferent ) {
    return calculateTheta2( ptReferent );
}
// Calculate Angle(A,B,C) by atan2
int GTPCC::calculateTheta1( Point ptReferent ) {
    if( !m_bOriginSet || !m_bRelatumSet) { // insufficient data 4 calc
        return false;
    }
...
int GTPCC::calculateTheta2( Point ptReferent ) {
    if( !m_bOriginSet || !m_bRelatumSet) { // insufficient data 4 calc
        return false;
    }
...
// GET Angle Segment
// =================================================================================
std::vector< int > GTPCC::calculateOrientationSegment() {
    vector< int > vResult;
    vResult.clear();
    int max = 4*m_iM;
    CAngle lb( 0.0 );
...
// =================================================================================
std::vector< int > GTPCC::calculateDistanceSegmentLinear() {
    vector< int > vResult;
    vResult.clear();
    float fDistProportion = 0.0;
    // calculate proportion between m_fRadius and Rel/Ref distance
    if( m_fRadius != 0 ) {
        fDistProportion = m_fDistRelRef / m_fRadius;
    }
    BB_DBG(2) << "Distance Proportion : " << fDistProportion << endl;
    // float lb, hb;

    if( fDistProportion == 0 ) {
        vResult.push_back( 0 );
        return vResult;
    }
    if( fDistProportion < 1 ) {
        vResult.push_back( 1 );
        return vResult;
    }
    vResult.push_back( 2 );
...
// =================================================================================
std::vector< int > GTPCC::calculateOrientationSegmentLinear() {
    vector< int > vResult;
    vResult.clear();
    int max = 4*m_iM;
    CAngle lb( 0.0 );
