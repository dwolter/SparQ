...
/* Class for getting qualitative results based on the GTPCC
 */
...
public:
    // constructor
    GTPCC( int m, float relatumX, float relatumY, float originX, float originY);
    GTPCC(int m);
    GTPCC();
    void GTPCCConstruct( int m,
    ...
    // of the point (dist, angle) relative to the relatum
    ...
    /* These methods are for naming the relations differently
       In the original GPCC relations are JEPD.
       In GTPCC linear relations are described by
